...
# 초당 요청 수 제어 (분당 40회 미만이므로, 1.6초당 1개씩 요청하면 분당 37.5회로 안전합니다)
...
async def generate_finance_data(row, semaphore):
    ...
    async with semaphore:
        ...
        try:
            ...
            print(f"\n[오류 발생 - 건너뜀] uuid: {row.get('uuid', '알수없음')} | {str(e)}")
            return None
...
async def main():
    ...
    # 동시 요청 수 제한 (한 번에 최대 5개씩만 요청하도록 설정)
    semaphore = asyncio.Semaphore(5)
    
    # 올바른 비율 제한(Rate Limit) 적용: 1.6초마다 하나씩 요청을 스케줄링합니다.
    tasks = []
    for _, row in df_sample.iterrows():
        tasks.append(asyncio.create_task(generate_finance_data(row, semaphore)))
