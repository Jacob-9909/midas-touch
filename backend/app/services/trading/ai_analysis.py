...
위 데이터를 기반으로 {ticker} 종목의 기술적·시장 지표를 해석한 정보성 리포트를 한국어 마크다운으로 작성하라.
특정 매매를 권유·추천하지 말고, 지표가 일반적으로 무엇을 시사하는지 설명하는 정보/교육 톤으로 작성하라.

포함 항목:
1. **기술적 지표 종합 해석** (지표들이 시사하는 방향과 그 근거)
...
간결하면서 실질적 정보를 제공하는 전문 리포트로 작성하라."""
