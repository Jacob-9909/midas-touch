...
기존 main.py / test_rag.py에 인라인돼 있던 GraphRAG 검색 로직을 도구로 격리한 것.
LLM 합성은 하지 않고, (그래프 관계망 트리플 + 근거 문서 본문) 컨텍스트만 반환한다.
...
def graph_rag(query: str) -> str:
    ...
    graph_context_str = "\n".join(sorted(set(subgraph_triplets))) if subgraph_triplets else "추출된 그래프 관계가 없습니다."

    unique_texts: list[str] = []
    for txt in text_contexts:
        if txt not in unique_texts:
            unique_texts.append(txt)
    text_context_str = "\n\n".join(unique_texts[:3]) if unique_texts else "관련 문서 본문이 없습니다."
