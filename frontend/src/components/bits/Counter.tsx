...
// React Bits / Counter — 오도미터형 자릿수 회전. 이식.
...
// 소수점은 훅을 쓰지 않는 별도 컴포넌트로 분리했다.
// 원본은 한 컴포넌트 안에서 place === '.' 일 때 조기 반환한 뒤 useSpring을 호출해
// 훅 규칙을 어겼다. 값이 정수 ↔ 소수로 바뀌면(1억 → 1.2억) places 구성이 달라지면서
// 같은 자리의 훅 순서가 변해 React가 터진다.
function DecimalPoint({ height, digitStyle }: Omit<DigitProps, 'place' | 'value'>) {
  return (
    <span
  ...
  return (
    <span style={{ ...defaultContainerStyle, ...containerStyle }}>
      <span style={{ ...defaultCounterStyle, ...counterStyle }}>
        {places.map((place, i) =>
          place === '.' ? (
            <DecimalPoint key={`dot-${i}`} height={height} digitStyle={digitStyle} />
          ) : (
